...
module.exports = {
	 ...
	 * @param {String} right
	 */
	CheckLenth: function (left, right) {
		if (left.length !== sails.config.auth.security.commonIdMathLength) {
			throw new Error("invalaid string length");
		}
		if (right.length !== sails.config.auth.security.commonIdMathLength) {
			throw new Error("invalaid string length");
		}
	},
	 ...
	 */
	BinaryAnd: function  (left, right) {
		let result = '';
		CheckLenth(left, right);
		for (let i = 0; i < sails.config.auth.security.commonIdMathLength; i++) {
	 ...
	 * @param {String} right 
	 */
	BinarySameOr: function (left, right) {
		let result = '';
		CheckLenth(left, right)
		for (let i = 0; i < sails.config.auth.security.commonIdMathLength; i++) {
			if (left[i] === right[i]) {
	 ...
	 * @return {String} result 
	 */
	BinaryNot: function (buff) {
		let result = ''; 
		if (buff.length !== sails.config.auth.security.commonIdMathLength) {
			throw new Error("invalaid string length");
		}
		for (let i = 0; i < sails.config.auth.security.commonIdMathLength; i++) {
			if (buff[i] === '0') {
	 ...
	 * @return {String} result
	 */
	BinaryChange: function (BinaryBuff) {
		let subBuff = '';
		let result = '';
		if (parseInt(BinaryBuff.length/4) != BinaryBuff.length/4) {
			throw new Error("BinaryBuff length invlade");
		}
		for (let i = 0; i < sails.config.auth.security.commonIdMathLength; i += 4) {
			// 截取特定长度的
	 ...
	 * @return {String} result
	 */ 
	GCD: function (decimalNum, bitLength) {
		let result = '';
		let mod;
		if (decimalNum === 0) {
